	...
	static function locationRegScheduleSQL()
	{
		...
		FROM regular_schedules s
			INNER JOIN services_at_location l
				ON s.service_id = l.service_id AND s.location_id = l.location_id
		';
	}
...
// ====== services ============================================================================	
	...
	static function serviceTaxonomySQL()
	{
		return 'SELECT 
			t.*, s.service_id
		FROM taxonomy t
	...
	function getServiceTaxonomyItem(string $id, string $item_id)
	{
		...
		$item_id = addslashes($item_id);
		$item = $this->connection->fetchAssoc(
					self::serviceTaxonomySQL() . 'WHERE s.service_id = :id AND t.id = :item_id',
					['id' => $id, 'item_id' => $item_id]
		);
		return $item;
	}
	...
	function getServicesTaxonomy($ids)
	{
		...
		if (is_array($ids))
			return $this->connection->fetchAll(
					self::serviceTaxonomySQL() . 'WHERE s.service_id IN (' . implode(',', $ids) . ')');
		elseif (is_string($ids))
			return $this->connection->fetchAll(
					self::serviceTaxonomySQL() . 'WHERE s.service_id = :id', ['id' => $ids]);
		else
			return [];
	}
	...
	static function serviceDetailSQL()
	{
		return 'SELECT d.*, s.service_id FROM details d INNER JOIN services_details s ON d.id=s.detail_id ';
	}

	function getServiceDetail(string $id, string $item_id)
	{
		$id = addslashes($id);
		$item_id = addslashes($item_id);
		$item = $this->connection->fetchAssoc(
					self::serviceDetailSQL() . 'WHERE s.service_id = :id AND d.id = :item_id',
					['id' => $id, 'item_id' => $item_id]
		);
		return $item;
	}
	
	function getServicesDetails($ids)
	{
		$ids = is_string($ids) ? addslashes($ids) : $ids;
		if ($ids === [] || $ids === '')
			return [];
		if (is_array($ids))
			return $this->connection->fetchAll(
					self::serviceDetailSQL() . 'WHERE s.service_id IN (' . implode(',', $ids) . ')');
		elseif (is_string($ids))
			return $this->connection->fetchAll(
					self::serviceDetailSQL() . 'WHERE s.service_id = :id', ['id' => $ids]);
		else
			return [];
	}
